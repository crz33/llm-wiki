...
このフォルダは**生ソース (不変・信頼できる情報源)**。LLM はここを読むだけで、書き換えることはない。例外は以下の 3 つのみ。

- `/ingest` による frontmatter の `status` 更新 (`stable → ingested`)
- `/meeting` による `meetings/` へのファイル新規作成
- `/note` による `notes/` `assets/` へのファイル新規作成、および note 側で更新された記事を再取り込みする際の当該ファイルの上書き (このとき `status` は `draft` に戻る。下記の遷移を参照)

ディレクトリ構成は以下。

- `assets/` : 各 markdown が参照する画像・音声・動画などのバイナリ資産。`/note` が記事中の画像をここへ保存する。
- `meetings/` : `/meeting` スキルが `vault/00_pools/sessions/` から抽出した議事録。
- `clips/` : Obsidian Web Clipper 等で取り込んだウェブ記事。
- `notes/` : `/note` スキルが note.com から取り込んだユーザ自身の公開記事、および手動で書いた生メモ・一次情報。
...
- `archived` : 生ソースとしては残すが Wiki には取り込まない状態。ユーザがこの状態に更新する。一般的すぎる情報、Wiki に載せる価値がないと判断したもの、古くなって参照しなくなったものが該当する。

遷移は `draft → stable → ingested`。ingest 済みのソースを直す場合はユーザが `draft` に戻す。

`archived` はこの流れから外れる終端状態で、どの状態からでもユーザの判断で移せる。`archived` のソースは `/ingest` の対象にならない (一覧に出さず、パス直接指定でも確認を取る)。取り込むことにした場合はユーザが `stable` に戻す。

例外として、`/note` が note.com の記事を再取り込み (上書き) したときは、生ソースの内容が変わるため `/note` が `status` を `draft` に戻す。その後の `stable` への更新はユーザの判断であり、`stable` になってから `/ingest` を再実行して Wiki 側に反映する。
...
各操作はスキルとして定義されており、`/ingest` `/query` `/lint` `/session` `/meeting` `/note` で起動する。具体的な手順は各スキル定義が持ち、本ドキュメントは対象レイヤーと責務のみを定める。
